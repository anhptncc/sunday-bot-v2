...
@Command('demmezonstar', {
  usage: '*demmezonstar',
})
export class DemMezonStarCommand extends CommandMessage {
  constructor(
  ...
  async getStars(): Promise<number> {
    const res = await firstValueFrom(
      this.http.get(`https://api.github.com/repos/mezonai/mezon`),
    );
  ...
  async execute(args: string[], message: ChannelMessage) {
    try {
      ...
      let url = this.cloudinary.getImageUrl(stars.toString());
